...
namespace TravelService.Controllers
{
    ...
    public class SharedEditController : ControllerBase
    {
        ...
        private readonly string _travelBase;
        private readonly string _activityBase;
        private readonly string _planningBase;

        public SharedEditController(AppDbContext context, IHttpClientFactory http, IConfiguration config)
        {
            _context = context;
            _http = http;
            _config = config;
            _travelBase = config["Services:TravelService"]!;
            _activityBase = config["Services:ActivityService"]!;
            _planningBase = config["Services:PlanningService"]!;
        }
        ...
        public Task<IActionResult> CreateActivity(string token, [FromBody] JsonElement body)
            => Proxy(token, HttpMethod.Post,
                tripId => $"{_activityBase}/api/trips/{tripId}/activities",
                body);
        ...
        public Task<IActionResult> UpdateActivity(string token, int activityId, [FromBody] JsonElement body)
            => Proxy(token, HttpMethod.Put,
                tripId => $"{_activityBase}/api/trips/{tripId}/activities/{activityId}",
                body);
        ...
        public Task<IActionResult> CreateChecklistItem(string token, [FromBody] JsonElement body)
            => Proxy(token, HttpMethod.Post,
                tripId => $"{_planningBase}/api/trips/{tripId}/checklist",
                body);
        ...
        public Task<IActionResult> ToggleChecklistItem(string token, int itemId, [FromBody] JsonElement body)
            => Proxy(token, HttpMethod.Patch,
                tripId => $"{_planningBase}/api/trips/{tripId}/checklist/{itemId}",
                body);
        ...
        public Task<IActionResult> CreateDestination(string token, [FromBody] JsonElement body)
            => Proxy(token, HttpMethod.Post,
                tripId => $"{_travelBase}/api/trips/{tripId}/destinations",
                body);
        ...
        public Task<IActionResult> UpdateDestination(string token, int destId, [FromBody] JsonElement body)
            => Proxy(token, HttpMethod.Put,
                tripId => $"{_travelBase}/api/trips/{tripId}/destinations/{destId}",
                body);
