using Common.DTOs;

namespace TravelService.Services
{
    public interface ITripService
    {
        Task<List<TripDto>> GetAllAsync(int userId);
        Task<TripDto?> GetByIdAsync(int id, int userId, bool isAdmin = false);
        Task<TripDto> CreateAsync(CreateTripDto dto, int userId);
        Task<TripDto?> UpdateAsync(int id, UpdateTripDto dto, int userId, bool isAdmin = false);
        Task<bool> DeleteAsync(int id, int userId, bool isAdmin = false);
        Task DeleteAllByUserAsync(int userId);
        Task<List<TripDto>> GetAllForAdminAsync();
    }
}